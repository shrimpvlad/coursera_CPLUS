...
struct AddressHasher {
    ...
    {
        size_t c = 2'946'901;
        return c*c*shash(adress.city) + c*shash(adress.street) + ihash(adress.building);
    }
    hash<string> shash;
    hash<int> ihash;
};
...
struct PersonHasher {
    ...
    {
        size_t c = 115'249;
        return c*c*c*shash(person.name) + c*c*ihash(person.height) + c*dhash(person.weight) + ahash(person.address);
    }
    hash<double> dhash;
    hash<string> shash;
    hash<int> ihash;
